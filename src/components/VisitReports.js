...
const mapStateToProps = state => ({
    visitReports: state.visitReportsData.visit_reports,
});
...
const VisitReports = ({ classes, visitReports }) => {
    ...
    const createRows = report => {
        const { agent_name,start_time, end_time, status, department_name, customer_name, number_of_tasks } = report;
        return({
            agent_name,
            start_time: dayjs(start_time._seconds * 1000).format('h: mm a, MMMM DD YYYY'),
            end_time: dayjs(end_time._seconds * 1000).format('h: mm a, MMMM DD YYYY'),
            status,
            department_name,
            customer_name,
            number_of_tasks,

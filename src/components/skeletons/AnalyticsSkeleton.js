...
const SupervisorsSkeleton = ({ classes }) => {

    const markup = Array.from({ length: 8 }).map((supervisor, supervisorIndex) => {
        return (
            <Card className={classes.card}>
                <CardMedia image={image} title="Profile" className={classes.image}/>
                <CardContent className={classes.content}>
                    <div className={classes.see}><span>
                        <Tooltip title="See Supervisor">
                            <IconButton>
...
export default withStyles(styles)(React.memo(SupervisorsSkeleton));

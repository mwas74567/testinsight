...
export const editInfo = newInfo => (async dispatch => {
    ...
    try {
        await axios.put('/supervisor/info', newInfo);
        dispatch({type: STOP_LOADING});
        dispatch(getUser());
    } catch(error) {
        console.error(error);
...
export const logoutUser = () => (dispatch => {
    localStorage.removeItem('Insights254AuthTokenSupervisor');
...
export const loginUser = (credentials, history) => (async dispatch => {
    ...
    try{
        ...
        if(decodedToken.userType !== 'supervisor'){
            dispatch({type: STOP_LOADING});
            dispatch({
                type: SET_ERRORS,
                payload: {error: 'User must be a recognized supervisor'}
            });
        }else {
            let fullToken = `Bearer ${token}`;
            localStorage.setItem('Insights254AuthTokenSupervisor', fullToken);
